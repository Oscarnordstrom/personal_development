...
# 2.3-7
The algorithm would not be improved, as the key value has to be swapped all values to the correct position even though the correct position is known at $\Theta(logn)$ time.
...
$\Theta (n logn)$ sort time, $\Theta(n)$ search time gives $\Theta(nlogn)$ overall time complexity.
