// New class definitions would be required, and also redefine deref function to
// be a const weak pointer.
...
public:
  ...
  ConstStrBlobPtr(StrBlob const &a, size_t sz = 0) : wptr(a.data), curr(sz) {}
...
std::string const &ConstStrBlobPtr::deref() const {
  auto p = check(curr, "dereference past end");
  return (*p)[curr];
}
