...
class Sales_data {
  ...
  friend double avg_price(const Sales_data &input_data);
...
double avg_price(const Sales_data &input_data) {
  return input_data.units_sold ? input_data.revenue / input_data.units_sold : 0;
}
...
int main() {
  ...
  cout << "average price for book 4: " << avg_price(book_4) << endl;
